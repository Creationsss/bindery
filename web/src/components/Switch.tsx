...
export default function Switch({ checked, onChange, label, children, title, className = '' }: SwitchProps) {
  return (
    ...
    >
      <span
        className={`relative w-9 h-5 rounded-full transition-colors flex-shrink-0 group-focus-visible:ring-2 group-focus-visible:ring-emerald-500 group-focus-visible:ring-offset-1 group-focus-visible:ring-offset-slate-100 dark:group-focus-visible:ring-offset-zinc-900 ${checked ? 'bg-emerald-600 group-hover:bg-emerald-500' : 'bg-slate-300 dark:bg-zinc-700 group-hover:bg-slate-400 dark:group-hover:bg-zinc-600'}`}
      >
